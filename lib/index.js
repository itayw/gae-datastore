...
gaed.lookup = function (datastore, collection, query, callback) {
  ...
  var projection = [];
  var filter = null;
  query.dimensions.forEach(function (dim) {
    projection.push({property: {name: dim.key || dim}});
  });
  query.metrics.forEach(function (met) {
    projection.push({property: {name: met.key || met}});
  });
  ...
  if (filter)
    theQuery.filter = filter;

  console.log(util.inspect(theQuery, {depth: null, colors: true}));
